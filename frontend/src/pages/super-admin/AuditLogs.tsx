...
function getBadgeVariant(action: string): "default" | "destructive" | "secondary" {
...
export default function SuperAdminAuditLogs() {
  ...
  useEffect(() => {
    apiClient("/api/admin/super/audit-logs")
  ...
  function exportCSV() {
    const rows = filtered.map((l) => ({
      ...
      target_id: String(l.target_id || "").slice(0, 8),
      details: l.details || "",
      actor: l.actor_name || String(l.actor_id || "").slice(0, 8),
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-10">
                {logs.length === 0
                  ? "No audit logs yet. Certificate events will appear here."
                  ...
                  <TableBody>
                    {filtered.map((l) => (
                      <TableRow key={l.id || l._id}>
                        <TableCell>
                          <Badge variant={getBadgeVariant(l.action)} className="text-[10px]">
                        ...
                        <TableCell className="text-xs text-muted-foreground hidden md:table-cell">
                          {l.actor_name || (l.actor_id ? String(l.actor_id).slice(0, 8) : "—")}
                        </TableCell>
